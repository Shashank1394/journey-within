export interface ItineraryDay {
  ...
  /**
   * Moves the illustration inside its grid column without affecting the text.
   * Use any CSS length, for example `{ x: "1rem", y: "-12px" }`.
   */
  imageOffset?: { x?: string; y?: string };
  /**
   * Moves the text inside its grid column without affecting the illustration.
   * Use any CSS length, for example `{ x: "-0.5rem", y: "1rem" }`.
   */
